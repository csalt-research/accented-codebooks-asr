...
if [ -z "$csvdir" ]; then
  echo "$0: CSV directory path not specified."
  exit 1;
fi

if [ ! -d "$csvdir" ]; then
  echo "$0: no such directory $csvdir"
  exit 1;
fi
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    local/create_mcv_data.sh $csvdir/train.tsv ${datadir}/clips/ data/${train_set}
    local/create_mcv_data.sh $csvdir/dev_small.tsv ${datadir}/clips/ data/${train_dev}
    local/create_mcv_data.sh $csvdir/test_small.tsv ${datadir}/clips/ data/${recog_set}
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    python scripts/add_accent_info.py --input-csv $csvdir/test_small.tsv  --input-json ${feat_recog_dir}/data_${bpemode}${nbpe}.json --output-json ${feat_recog_dir}/data_${bpemode}${nbpe}_with_accent.json
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    ...
    ${cuda_cmd} --gpu ${ngpu} ${expdir}/train.log \
        ...
        --verbose ${verbose} \
        --resume ${resume} \
        --train-json ${feat_tr_dir}/data_${bpemode}${nbpe}_with_accent.json \
        --valid-json ${feat_dt_dir}/data_${bpemode}${nbpe}_with_accent.json
fi
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    ...
    for rtask in ${recog_set}; do
    (
        decode_dir=decode_${rtask}_$(basename ${decode_config%.*})_${lmtag}_epoch_49
        feat_recog_dir=${dumpdir}/${rtask}/delta${do_delta}

        # split data
        splitjson.py --parts ${nj} ${feat_recog_dir}/data_${bpemode}${nbpe}_with_ivector.json
        ...
        ${decode_cmd} JOB=1:${nj} ${expdir}/${decode_dir}/log/decode.JOB.log \
            ...
            --backend ${backend} \
            --batchsize 0 \
            --recog-json ${feat_recog_dir}/split${nj}utt/data_${bpemode}${nbpe}_with_ivector.JOB.json \
