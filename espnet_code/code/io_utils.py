...
class LoadInputsAndTargets(object):
    ...
    Added the accent labels option on my own
    """
    ...
    def __call__(self, batch, return_uttid=False):
        ...
        for uttid, info in batch:
            ...
            if self.load_output:
                ...
                x = np.array(info["accent"])
                y_acc_labels_dict.setdefault("label1", []).append(x)
        ...
        if self.mode == "asr":
            return_batch, uttid_list = self._create_batch_asr(
                x_feats_dict, y_feats_dict, y_acc_labels_dict, uttid_list
            )
        elif self.mode == "tts":
            _, info = batch[0]
    ...
    def _create_batch_asr(self, x_feats_dict, y_feats_dict,y_acc_labels_dict,uttid_list):
        ...
        :param OrderedDict y_acc_labels_dict:
            e.g. {"label1":[int, int, ...]}
        ...
        if self.load_output:
            ...
            y_labels = list(y_acc_labels_dict.values())
        ...
        if self.load_output:
            ...
            y_labels = [[yl[i] for i in nonzero_sorted_idx] for yl in y_labels]
            y_label_names = list(y_acc_labels_dict.keys())
            ...
            return_batch = OrderedDict(
                [
                    ...
                    *[(y_label_name, y_label) for y_label_name, y_label in zip(y_label_names, y_labels)],
                ]
            )
        else:
            return_batch = OrderedDict([
