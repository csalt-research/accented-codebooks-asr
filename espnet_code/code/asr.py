...
from asr_init import freeze_modules
from asr_init import load_trained_model
from asr_init import load_trained_modules
...
class CustomConverter(object):
    ...
    def __call__(self, batch, device=torch.device("cpu")):
        ...
        ilens = numpy.array([x.shape[0] for x in xs])
        ...
        ys_pad = pad_list(
            [
                torch.from_numpy(
                    numpy.array(y[0][:]) if isinstance(y, tuple) else y
                ).long()
                for y in ys
            ],
            self.ignore_id,
        ...
        acc_labels = torch.from_numpy(numpy.asarray(acc_label)).to(device)

        # return xs_pad,ilens,ys_pad
        return xs_pad, ilens, ys_pad, acc_labels

class CustomConverterActivityConditionedSingle(object):
    """Custom batch converter for Pytorch.
    ...
    def __call__(self, batch, device=torch.device("cpu")):
        ...
        # perform subsampling
        if self.subsampling_factor > 1:
            xs0 = [x[:: self.subsampling_factor, :] for x in xs0]
            xs1 = [x[:: self.subsampling_factor, :] for x in xs1]

        # get batch of lengths of input sequences
        ilens = np.array([x.shape[0] for x in xs0])

        # perform padding and convert to tensor
        # currently only support real number
        if xs0[0].dtype.kind == "c":
            raise ValueError('not supported')
        else:
            xs0_pad = pad_list([torch.from_numpy(x).float() for x in xs0], 0)
        xs1_pad = pad_list([torch.from_numpy(x).float() for x in xs1], 0)
        xs_pad = torch.cat((xs0_pad,xs1_pad), dim=-1).to(device)
...
class CustomConverterActivityConditioned(object):
    """Custom batch converter for Pytorch.
    ...
    def __call__(self, batch, device=torch.device("cpu")):
        ...
        xs_pad = torch.cat((torch.cat((xs0_pad,xs1_pad[:,:,:1]), dim=-1), torch.cat((xs0_pad,xs1_pad[:,:,1:2]), dim=-1)), dim=0).to(
                device, dtype=self.dtype
            )

        ilens = torch.cat((torch.from_numpy(ilens), torch.from_numpy(ilens))).to(device)
        # NOTE: this is for multi-output (e.g., speech translation)
        ys_pad = pad_list(
            [
                torch.from_numpy(
...
def train(args):
    ...
    # Setup a converter
    if args.model_type=='activity_conditioned':
        converter = CustomConverterActivityConditioned(subsampling_factor=model.subsample[0], dtype=dtype)
    if args.model_type=='activity_conditioned_single':
        converter = CustomConverterActivityConditionedSingle(subsampling_factor=model.subsample[0], dtype=dtype)
    else:
        if args.num_encs == 1:
