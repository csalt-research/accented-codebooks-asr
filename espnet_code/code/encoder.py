...
class Encoder(torch.nn.Module):
    ...
    Args:
        ...
        intermediate_layers (Union[List[int], None]): indices of intermediate CTC layer.
            ...
            signature.)
        use_codebooks (bool): Whether to use codebook based cross-attention or not.
        no_accents (int): No of accents used for training.
        codebooks_per_accent (int): No of codebook entries used per accent.
        codebook_cross_attention_layers (Union[List[int], None]): indices of layers where codebook based cross attention is to be added.
            indices start from 1. It is relevant only when `use_codebooks` is set to True.
    """
    ...
    def __init__(
        ...
        use_codebooks=False,
        no_accents=5,
        codebooks_per_accent=50,
        codebook_cross_attention_layers=None
    ):
        ...
        self.encoders = repeat(
            ...
            lambda lnum: EncoderLayer(
                ...
                int(1+lnum) in codebook_cross_attention_layers,
            ),
        )
        ...
        logging.warn(f"Using intermediate layers: {self.use_conditioning}, {self.intermediate_layers}")
    ...
    def forward(self, xs, masks, accent_labels):
        ...
        Args:
            ...
            accent_labels (torch. Tensor): Accent labels tensor (#batch, ).
        ...
        if self.intermediate_layers is None:
            xs, masks, _ = self.encoders(xs, masks, codebooks)
        else:
            intermediate_outputs = []
            for layer_idx, encoder_layer in enumerate(self.encoders):
                xs, masks, _ = encoder_layer(xs, masks, codebooks)
